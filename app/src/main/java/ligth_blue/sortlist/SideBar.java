...
public class SideBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < b.length; i++) {
            paint.setColor(getResources().getColor(R.color.black));//字体颜色
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_UP://手指抬起时，设置为透明颜色
...
//                setBackground(getResources().getColor(R.color.white));
                ...
                invalidate();
                if (mTextDialog != null) {
                    mTextDialog.setVisibility(View.GONE);//隐藏dialog
                }
...
////                    float xPos = width / 2 - paint.measureText(b[i]) / 2;
////                    float yPos = singleHeight * i + singleHeight;
////                    getcan.drawText(b[i], xPos, yPos, paint);
//                    paint.reset();// 重置画笔
//                }
                break;
