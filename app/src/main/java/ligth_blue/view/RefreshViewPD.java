 ...
 * 配合ListView使用时,请调用setListViewScrollListener() 或者 给ListView设置onScrollListener并且在OnScroll方法中调用onListViewScroll()
 * @author planet
 *
 */
public class RefreshViewPD extends FrameLayout {
	...
	private boolean pullDownOpen = false;
	private boolean pullUpOpen = false;
	...
	public RefreshViewPD(Context context, AttributeSet attrs) {
		super(context, attrs);
		mTouchSlop = 5;
		mScroller = new Scroller(context);
		init(context);
	}
	public RefreshViewPD(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs);
		mTouchSlop = 5;
		mScroller = new Scroller(context);
		init(context);
	...
	private void init(Context context){
		...
		LayoutParams lp = new LayoutParams(LayoutParams.FILL_PARENT, headerheight);
		...
		*/
		/*progressBar = new IndeterminateProgressBar(context);
		progressBar.setColor(0xffcccccc);*/
		//progressBar.setVisibility(View.INVISIBLE);
		//addViewInLayout(progressBar, 0, lp);
		lp = new LayoutParams(dip2px(46), dip2px(46));
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		header.layout(header.getLeft(), header.getTop()-headerheight, header.getRight(), header.getBottom()-headerheight);
		footer.layout(footer.getLeft(), footer.getTop()+headerheight, footer.getRight(), footer.getBottom()+headerheight);
	}
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		//Log.i(TAG, "dispatchTouchEvent>>>>> enablePullDown="+enablePullDown+" enablePullUp="+enablePullUp);
		checkTouch(ev);
			...
			if(ev.getAction() == MotionEvent.ACTION_MOVE){
				if(!mScolling){
					...
					if(Math.abs(slop)>=mTouchSlop){
						...
						if(grad<1 && ((slop>=0 && enablePullDown) || (slop<=0 && enablePullUp))){	//开始scroll
							mScolling = true;
							pullDown = slop>=0;
							ev.setAction(MotionEvent.ACTION_CANCEL);
							super.dispatchTouchEvent(ev);
						}else{//如果slop正负与滑动方向不一致 或者在其他情况下 取消拉动监听
							if(!atListViewTop) enablePullDown = false;
							if(!atListViewBottom) enablePullUp = false;
						}
					}
				}else{
					//不允许倒着滚动
					if(pullDown && getScrollY()<=0 || !pullDown && getScrollY()>=0){
						//根据初次触摸的Y和当前移动的Y的差, scroll相同的距离
						float scrollY = eventFloatY - mLastPoint.y;
						scrollTo(0, -(int) (scrollY/2.5));
						return true;
					}else{
						scrollTo(0, 0);
						mScolling = false;
	...
	/**
	 * 如果您已经设置了ListView的setOnScrollListener，请在onScroll中调用此方法
	 * 此方法用来设置下拉、上拉的使能状态: 如果ListView滑动到顶部,使能PullDown,如果ListView滑动到底部使能PullUp
	 */
	public void onListViewScroll(ListView listView){
		if(listChild == null) listChild = listView;
		if(listView.getAdapter()==null || listView.getAdapter().getCount() == 0){
			if(pullUpOpen) enablePullUp = true;
			atListViewBottom = true;
			if(pullDownOpen) enablePullDown = true;
			atListViewTop = true;
			return;
		}
